...
class OverlaySprite {
    constructor(src, scale={x:0,y:0}, mask_color="#000000"){
        this.img = new Image();
        this.img.src = src;
        this.scale = scale;
        this.opacity = 1;
        ...
        this.img.addEventListener('load', () => {
            ...
            this.set_mask_color(this.mask_color);
            this.loaded = true;
            this.onload();
        });
    }
    set_mask_color(color){
        ...
        ctx.drawImage(this.img, 0, 0, this.scale.x, this.scale.y);
        const img_data = ctx.getImageData(0, 0, this.scale.x, this.scale.y).data;
        this.data = ctx.createImageData(this.scale.x, this.scale.y);
        img_data
        for (let i = 0; i < this.data.data.length; i += 4) {
            if((img_data[i] > 200 && img_data[i  + 1] < 100 && img_data[i + 2] > 150 )&& img_data[i + 3] > 0){
                // Modify pixel data
                this.data.data[i + 0] = parseInt(this.mask_color.substring(1, 3), 16); // R value
                this.data.data[i + 1] = parseInt(this.mask_color.substring(3, 5), 16); // G value
                this.data.data[i + 2] = parseInt(this.mask_color.substring(5, 8), 16); // B value
                this.data.data[i + 3] = 255; // A value
            }else{

                this.data.data[i + 0] = img_data[i  + 0]; // R value
                this.data.data[i + 1] = img_data[i  + 1]; // G value
                this.data.data[i + 2] = img_data[i  + 2]; // B value
                this.data.data[i + 3] = img_data[i  + 3]; // A value 
            }
    ...
    render(ctx, pos){
        ...
        // Draw the image, offsetting by half the width and height
        ctx.drawImage(this.canvas, -this.scale.x / 2, -this.scale.y / 2, this.scale.x, this.scale.y);
